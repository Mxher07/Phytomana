...
namespace Game {
    public class SubsystemGrownDaisyBehavior : SubsystemBlockBehavior, IUpdateable {
        ...
        public void ApplyConversion(ConvertData conversion) {
            ...
            if (SubsystemTerrain.Terrain.GetCellContents(x, y, z) == conversion.SourceContents) {
                SubsystemTerrain.ChangeCell(x, y, z, Terrain.MakeBlockValue(conversion.DestinationContents, 0, 0));
            }
        ...
        public void ScanAroundDaisy(DaisyData daisy) {
            ...
            for (int i = x - 1; i <= x + 1; i++) {
                for (int j = z - 1; j <= z + 1; j++) {
                    TerrainChunk cellChunk = SubsystemTerrain.Terrain.GetChunkAtCell(i, j);
                    if (cellChunk == null || cellChunk.State != TerrainChunkState.Valid) {
                        continue;
                    }
                    Point3 point = new(i, y, j);
                    int cellContents = SubsystemTerrain.Terrain.GetCellContents(i, y, j);
                    ...
                    if (m_sourceToDestination.TryGetValue(cellContents, out int destinationContents)) {
                        ...
                        && conversion.DaisyPoint == daisy.Point) {
